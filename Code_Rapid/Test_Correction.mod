MODULE Test_Correction
    !***********************************************************
    !
    ! Module:  Test_Correction
    !
    ! Description : Base de code avec asservissement et correction de la coordonnée Z avec signal de capateur de distance en entree.
    !
    ! Auteur : Nahkriin
    !
    ! Version : 1.0
    ...
    CONST num Z_obj := 0; !La valeur objectif de la coordonnée en Z est fixée à 0
    CONST num G := 2; !Gain du correcteur
    
    VAR num T := 0.2; !Période (s) de lecture du signal du capteur
    
    VAR intnum ID_1;    !Identifiant de la routine de correction
    
    VAR corrdescr z_id; !Variables de correction
    VAR pos write_offset;
    VAR pos total_offset;
    
    VAR num z_sign; !Signal du capteur : renvoie la distance observée entre l'outil et la surface plane de la trajectoire
    ! Grosse problématique : Comment mettre à jour ces valeurs ? On peut inserer une MaJ a chaque declenchement de la routine de correction, mais comment faire ?
    ...
    TRAP routine
        ...
        IF Z_sign <= Z_obj THEN !Discretisation des cas de correction positive et negative
            write_offset.z := (Z_obj-Z_sign)*G;
        ELSE
            write_offset.z := (Z_sign-Z_obj)*G;
        ENDIF
        ...
        !IDelete ID_1;
        !CONNECT ID_1 WITH routine;
        !ITimer\single, T ID_1;
    ENDTRAP

    PROC main()
        
        CorrCon z_id;   !Connection du capteur
        
        CONNECT ID_1 WITH routine;
        ITimer T, ID_1; !Pour l'instant, on définit l'interrupt de manière cyclique, mais ce sera peut etre a corriger avec un /Single et un reamorcage dans la TRAP...
        
        ConfJ \Off;
        ConfL \Off;
        MoveAbsJ Targ0,MySpeed,z1,Tool0; 
        
        TPWrite "Starting routine."; 
