...
int main(int argc, char **argv)
{
	...
	*/
	...
	int mess_length;
	int iResult;
	char * adresse = "localhost";

	client connection_capteur;
	iResult = connection_capteur.initialization(argv[1], "27015");
	if (iResult != 0) {
		return iResult;
	}

	iResult = connection_capteur.send_message("plop");
	if (iResult == 1) {
		return 2;
	}

	iResult = connection_capteur.recv_message(mess, mess_length);
	if (iResult == 1) {
		return 3;
	}
